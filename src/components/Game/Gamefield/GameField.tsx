...
export default function GameField() {
  ...
  useEffect(() => {
    ...
    newSocket.on("gameOver", (body: any) => {
      ...
      if (body.username === username) {
        navigate("/win"); // noch einbauen
      } else {
        navigate("/loose"); // einbauen
      }
    });
    newSocket.on("enabledPlayer ", (body: any) => {
      setEnabledPlayer(body.username);
    });
  ...
  const generateShips = (ships: ShipTemplate[]) => {
    ...
    for (let i = 0; i < ships.length; i++) {
      ...
      // console.log(s.identifier, startColumn, endColumn, startRow, endRow);
      divs.push(
        <div
          style={{
  ...
  const whichTurn: boolean = username === enabledPlayer;

  const changeEnabledPlayer = (secondUser: string) => {
    // if (enabledPlayer == username) setEnabledPlayer("ki");
    // else setEnabledPlayer(username);
    setEnabledPlayer(secondUser);
